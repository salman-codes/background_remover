...
class U2NET:
    ...
    def process_image(self, data, preprocessing=None, postprocessing=None):
        """
        The function removes background from image and returns RGBA Image (PIL type).
            :required parameter data: Takes image with path
            :optional parameter preprocessing: Image Pre-Processing Algorithm Class
            :optional parameter postprocessing: Image Post-Processing Algorithm Class
        :return: PIL RGBA Image. If any error in detecting or loading the image, then the function returns False.
        """
        if isinstance(data, str):
            logger.debug("Load image: {}".format(data))

        # Loading image
        image, org_image = self.__load_image__(data)
        if image is False or org_image is False:
            return False
        if preprocessing:  # If the preprocesses algorithm is specified,
            # Then this algorithm will immediately remove the background
            image = preprocessing.run(self, image, org_image)
        else:
            image = self.__get_output__(image, org_image)  # If the above doesn't work, then just remove the background
        if postprocessing:  # If the postprocessing algorithm is specified, we send it an image without a background
    ...
    def __get_output__(self, image, org_image):
        ...
        :required parameter image: Prepared Image
        :required parameter org_image: Original pil image
    ...
    def __load_image__(self, data):
        ...
        :required parameter data: Path to image file or PIL image
    ...
    def __ndrarray2tensor__(self, image: np.ndarray):
        ...
        :required parameter image: Image numpy array
        :return: Image tensor
        """
...
class BasNet:
    ...
    def process_image(self, data, preprocessing=None, postprocessing=None):
        ...
        :required parameter data: Path to image or PIL image
        :optional parameter preprocessing: Image Pre-Processing Algorithm Class
        :optional parameter postprocessing: Image Post-Processing Algorithm Class
